...
export class NewMessagePageComponent extends FormValidation implements OnInit, OnDestroy, AfterViewInit {
  ...
  socketTopicmessage !: Subscription;
  socketTopicmessage1!: Subscription;
  socketTopicmessage2!: Subscription;
  socketTopicmessage3!: Subscription;
  socketTopicmessage4!: Subscription;
  socketTopicmessage6!: Subscription;
  socketTopicmessage7!: Subscription;
  socketTopicmessage8!: Subscription;
  socketTopicmessage9!: Subscription;
  socketTopicmessage10: Subscription;
  constructor(private _api: ApiService, private _util: UtilService, private _location: Location,
    ...
    private JobServicecolor: JobService,private cdr: ChangeDetectorRef ,
  private streamService :SocketServiceStream) {
    ...
    this.modalOptions = {
      class: 'group-msg-modals modal-md',
      backdrop: 'static',
      keyboard: false
    };
  ...
  //   this.chatHeight = this.innerHeight - 200;
  // }

  @HostListener('window:resize', ['$event'])
  onResize1(event) {
    this.innerHeight1 = window.innerHeight;
    this.chatHeight1 = this.innerHeight1 - 90;
  }
  ...
  summernoteForm: UntypedFormGroup;
  ngOnInit() {

    this.contact.searchAfterKey=null
    this.innerHeight = window.innerHeight;
    ...
    this.summernoteForm = new UntypedFormGroup({
      summernoteEditor: new UntypedFormControl('', [this.summernoteValidator]),
    });
  ...
  summernoteValidator(control: UntypedFormControl) {
    const content = control.value;
    // Check if the content is empty or just contains HTML tags
  ...
  onSubmitsummernote() {
    if (this.summernoteForm.invalid) {
      // Handle the invalid form.
      console.log('Form is invalid due to empty content');
    } else {
  ...
  loginuser() {
    this._api.query("user/" + localStorage.getItem('userId')).subscribe((res) => {
      if (res.code == "00000") {
  ...
  ngOnDestroy() {
    ...
    if(this.socketTopicmessage){
      this.socketTopicmessage.unsubscribe();
    }

    if(this.socketTopicmessage1){
      this.socketTopicmessage1.unsubscribe();

    }
    if(this.socketTopicmessage2){
      this.socketTopicmessage2.unsubscribe();

    }
    if(this.socketTopicmessage3){
      this.socketTopicmessage3.unsubscribe();

    }
    if(this.socketTopicmessage4){
      this.socketTopicmessage4.unsubscribe();

    }
    if(this.socketTopicmessage6){
      this.socketTopicmessage6.unsubscribe();

    }
    if(this.socketTopicmessage7){
      this.socketTopicmessage7.unsubscribe();

    }
    if(this.socketTopicmessage8){
      this.socketTopicmessage8.unsubscribe();

    }
    if(this.socketTopicmessage9){
      this.socketTopicmessage9.unsubscribe();

    }
    if(this.socketTopicmessage10){
      this.socketTopicmessage10.unsubscribe();
  ...
  private getAllContacts() {
    ...
    this._api.messagePageService('GET', url).subscribe(res => {
     ...
     let newdatas:any  = res.filter(element => element.groupName != undefined);
     console.log(newdatas);

      // let  newres = res.filter(element => element.groupName !== undefined || element.groupName !== null);
      newdatas.forEach(element => {
          if (element.lastMessageOn == null) {
            element.lastMessageOn = element.createdOn
          }
          this.contactList = newdatas;
        });
  ...
  private queryParamBasedMethod() {
    ...
    if (type && type != null && type != '') {
      ...
      this._api.messagePageService('POST', url, contact).subscribe(res => {
        ...
        if (res && res['code'] == '00000') {
          ...
          if (d['exist'] || d['groupType']=="SUPPORT" && res.groupId!=undefined &&  res.groupId!=null) {
            if (res.groupStatus == "INACTIVE") {
              Swal.fire('', 'The ' + res.groupType.toLowerCase() + ' has been deleted', 'error');
            }
            if ((type === 'COMMUNITY' || type === 'BUSINESS'|| type === 'SUPPORT') && (groupId == undefined || groupId == null || groupId == '')) {
              let l = {
                groupId: d['groupId'],
                type: type,
                refererId: refererId
            ...
            if (type === "BUSINESS" || type === "COMMUNITY" ||  type === "COMMUNITY") {
          ...
          this.socketTopicmessage10 =  this._socket.onMessage('/topic/message/' + groupId + '-' + localStorage.getItem('userId')).subscribe(res => {
              //////"USER", res)
           ...
           this.socketTopicmessage9 = this._socket.onMessage('/topic/message/' + localStorage.getItem('userId') + '-' + groupId).subscribe(res => {
              //////"USER2", res)
              //////" this.SelectedContact", this.SelectedContact)
              if (this.SelectedContact.groupId == res.authorId && type === 'USER') {
          ...
          } else if (type === 'GROUP' || type === 'NETWORK' || type === 'TEAM' || type === 'JOB' || type === 'CANDIDATE'  ) {

          this.socketTopicmessage8 =  this._socket.onMessage('/topic/message/' + groupId).subscribe(res => {
              //////"GROUP1", res);
              //////res);
              //////"STATUS",this.SelectedContact.status)
              if (type === 'GROUP' || type === 'NETWORK' || type === 'TEAM' || type === 'JOB' || type === 'CANDIDATE'  ) {
                if (res.eventType == "ACTION") {
                  var dates = new Date(res.sentDate);
                  let latest_date = this.datepipe.transform(dates, 'EEE, MM/dd/yyyy yy h:mm:ss a');
                  var tempMsg: any = [];
           ...
           this.socketTopicmessage7  = this._socket.onMessage('/topic/event/' + localStorage.getItem('userId')).subscribe(res_evn => {
              //////"GROUP2", res)
          ...
          } else if (type === 'BUSINESS' || type === 'COMMUNITY' ||  type === 'SUPPORT') {

            if (groupId != undefined && groupId != null && groupId != "") {

           this.socketTopicmessage6  =  this._socket.onMessage('/topic/message/' + groupId).subscribe(res => {
                ...
                if (res.recipientId == this.SelectedContact.groupId && type === 'BUSINESS' || res.recipientId == this.SelectedContact.groupId && type === 'COMMUNITY') {
                  ...
                  if (res.eventType == "ACTION") {
                    var dates = new Date(res.sentDate);
                    let latest_date = this.datepipe.transform(dates, 'EEE, MM/dd/yyyy h:mm:ss a');
            ...
            this.socketTopicmessage4 =   this._socket.onMessage('/topic/message/' + refererId).subscribe(res => {
          ...
          this.socketTopicmessage3 =  this._socket.onMessage('/topic/event/' + localStorage.getItem('userId')).subscribe(res_evn => {
    ...
    this.socketTopicmessage2 =   this._socket.onMessage('/topic/contacts/all/' + localStorage.getItem('userId')).subscribe((contact) => {
      //////'123 ', contact);
    ...
    this.socketTopicmessage1 =  this._socket.onMessage('/topic/contacts/all/remain/' + localStorage.getItem('userId')).subscribe((contact) => {
      let contactList = contact;
    ...
    this.socketTopicmessage =  this._socket.onMessage('/topic/contacts/' + localStorage.getItem('userId')).subscribe((contact) => {
      ...
      if (this.contactList != null && this.contactList != undefined) {
        ...
        this.contactList.forEach((e, index) => {
          ...
          if (contact.groupId === e.groupId) {
            //
            if(e['online'] != undefined && contact['online'] != undefined && (e['online'] != contact['online'])){
              e['online'] = contact['online'];
             }
            else{
  ...
  groupMembersList = [];
  getGroupMembers(data, template, openModal, payload = this.contact) {
    const groupType = this.aroute.snapshot.queryParamMap.get('type') || this.SelectedContact.groupType;
    ...
    this.groupUserList=[];
    this._util.startLoader('msgspinner');
    this._api.messagePageService('POST', "message/group/getGroupMembers", payload).subscribe(res => {
      this._util.stopLoader('msgspinner');

      if (res.code === GigsumoConstants.SUCESSCODE) {
        const { data } = res;
        const key = res.data.searchAfterKey && res.data.searchAfterKey[0];
        const newMembers = data.groupMembers.filter((newMember) =>
          !this.groupUserList.some((existingMember) => existingMember.userId === newMember.userId)
        );

        if (this.previousSearchAfterKey === key || (data.groupMembers && data.groupMembers.length === 0)) {
          this.stopScroll = true;
        }

        if (this.previousSearchAfterKey !== key || (data.groupMembers && data.groupMembers.length > 0)) {
          this.groupUserList = [...this.groupUserList, ...newMembers];
          this.TempGroupUserList = data;
        }
          this.previousSearchAfterKey = key;
        if (this.modalRef) {
          this.modalRef.hide();
        }
        if (this.modalRefAddgroup) {
          this.modalRefAddgroup.hide();
        }
        if (openModal != undefined && openModal != null && openModal != '' && openModal) {
         this.modalRef = this.modalService.show(template, Object.assign({}, this.modalOptions));
          document.body.classList.add('body-no-scroll');
        }
      }
    }, err => this._util.stopLoader());
  }
  ...
  LoadmoreGroupMembers() {
    ...
    Object.assign(this.contact, {
      ...
      groupType: groupType,
      limit: 10,
      searchContent:this.searchContent===''?null:this.searchContent,
     });
     this._util.startLoader('msgspinner');
    this._api.messagePageService('POST', "message/group/getGroupMembers", this.contact).subscribe(res => {
      this._util.stopLoader('msgspinner');
      if (res.code === GigsumoConstants.SUCESSCODE) {
        const { data } = res;
        const key = res.data.searchAfterKey && res.data.searchAfterKey[0];
        ...
        if (this.previousSearchAfterKey !== key) {
          if (this.searchContent != '') {
            this.groupUserList=data.groupMembers;
          } else {
            this.groupUserList = [
              ...this.groupUserList,
              ...data.groupMembers.filter((newMember) => !this.groupUserList.some((existingMember) => existingMember.userId === newMember.userId))];
            this.TempGroupUserList = data;
          }
        }
        this.previousSearchAfterKey = key;
  ...
  BusinessGrouplist(data, template, openModal) {
    ...
    this._api.messagePageService('POST', url, contact).subscribe(res => {
      this._util.stopLoader('msgspinner');
      if (res) {
        res.data.groupMembers.forEach(element => {
       ...
       if (this.previousSearchAfterKey !== key) {
        this.groupBusinessListMembers = res.data.groupMembers;
        this.tempBusinessList = res.data.groupMembers;
       }
       this.previousSearchAfterKey = key;
        if (openModal != undefined && openModal != null && openModal != '' && openModal) {
          this.modalRef = this.modalService.show(template, Object.assign({}, this.modalOptions));
  ...
  searchgetMembersForGroup() {

    if (this.searchContent.trim() !== '') {
      this.contact.searchAfterKey = null
      this.previousSearchAfterKey = null
      this.searchandscrollconatcts();
    } else {
      this.newContactList = [];
      this.newuserGroup = [];
      this.tempNewuserGroup = [];
      this.tempNewContactList = [];
      this.contact.searchAfterKey = null
      this.previousSearchAfterKey = null
      this.searchandscrollconatcts();
    }
  ...
  onScrollcontacts(){
    ...
    if (!this.stopScroll) {
      this.contact.searchAfterKey = this.previousSearchAfterKey === null ? null
      : [this.previousSearchAfterKey];
     this.searchandscrollconatcts();
  }
  }
  ...
  onsearchgroupmembers(){
    if (this.searchContent.trim() !== '') {
      this.contact.searchAfterKey = null
      this.previousSearchAfterKey = null
      this.LoadmoreGroupMembers();
    } else {
      this.groupUserList = [];
      this.contact.searchAfterKey = null
      this.previousSearchAfterKey = null
  ...
  filterByStringbybusiness(data: any[], s: string) {
  ...
  searchandscrollconatcts(){
      ...
      this._util.startLoader('msgspinner');
      this._api.messagePageService('POST', url, this.contact).subscribe(res => {
        this._util.stopLoader('msgspinner');
        if (res) {
          res.data.connections.forEach(element => {
            element.checked = false;
            element.groupName= element.firstName+" "+element.lastName;
            element.groupId=element.userId;
          });
          const key = res.data.searchAfterKey && res.data.searchAfterKey[0];
          if (this.previousSearchAfterKey === key || (res.data.connections && res.data.connections.length==0)) {
           this.stopScroll = true
         }
         if (this.previousSearchAfterKey !== key) {

          if (this.searchContent != '') {
            this.newContactList = res.data.connections;
            this.tempNewContactList = res.data.connections;
            this.newuserGroup=res.data.connections;
            this.tempNewuserGroup=res.data.connections;
          } else {
            this.newContactList = this.newContactList.concat(res.data.connections);
            this.newuserGroup = this.newuserGroup.concat(res.data.connections);
            this.tempNewuserGroup = this.tempNewuserGroup.concat(res.data.connections);
            this.tempNewContactList = this.tempNewContactList.concat(res.data.connections);
          }
         }
          this.previousSearchAfterKey = key;
        }
  ...
  addcontact(openModal:any,templates?: TemplateRef<any>, businesstemplates?: TemplateRef<any>, communityMembertemplate?: TemplateRef<any>) {

    if (this.SelectedContact.groupType == 'GROUP'||openModal==true) {
      let url = "message/group/getMembersForGroup";
      let contact = {
        userId: localStorage.getItem('userId'),
        groupId: this.SelectedContact.groupId,
      ...
      this._api.messagePageService('POST', url, contact).subscribe(res => {
        ...
        if (res) {
          ...
          if (openModal != undefined && openModal != null && openModal != '' && openModal && this.newContactList.length != 0) {
            this.modalRef = this.modalService.show(templates, Object.assign({}, this.modalOptions));

           }
          // else {
          //   Swal.fire('', 'No user Connection', 'error');
          // }
          if (res.data.connections.length != 0 &&!openModal) {
            this.modalRefAddgroup = this.modalService.show(templates, Object.assign({}, { class: 'group-msg-modals modal-md' }));
          }
          // else {
          //   Swal.fire({
          //     position: 'center',
  ...
  private constructMsg(that: any, msg: any) {
    ...
    if (msg.enableDelete == true && msg.authorId == this.userId) {
      ...
      msg.authorPhoto = this.loginUserData.photo;
      msg.authorName = this.loginUserData.firstName + " " + this.loginUserData.lastName;
    ...
    if (this.chartDisc) {
      setTimeout(() => {
        if (this.chartDisc.nativeElement) {
          this.chartDisc.nativeElement.scroll({
            top: this.chartDisc.nativeElement.scrollHeight,
            left: 0,
            behavior: 'smooth'
  ...
  getNewContact(openModal?: any, template?: TemplateRef<any>) {
      ...
      this._api.messagePageService('POST', url, this.sendMessageData).subscribe(res => {
        ...
        if (res) {
          if (res.code === "00000") {
            ...
            if (this.userRedirect.groupId == null && this.userRedirect.groupId == undefined && (type == "BUSINESS" || type == "COMMUNITY"||  type == "SUPPORT")) {
  ...
  updatemediaApi() {
    ...
    this.uploadFiles.forEach(element => {
      ...
      console.log("dataa addded", this.uploadFiles);
    ...
    this._api.messagePageService('POST', url, this.FormData).subscribe(res => {
      ...
      if (res.code == "00000") {
        this.uploadFiles = [];
        this.editValueEditImage = [];

        this.messageList.forEach(element => {

          if (element.messageId == this.editValue.messageId) {
            element.message = this.editValue.message;
            element.attachments =  res.data.updatedMessage.attachments.filter(item => 'messageId' in item);
            this.summernote.writeValue('');
            this.editValue = "";
  ...
  reloadcontact() {
    ...
    this._api.messagePageService('GET', url).subscribe(res => {
      if (res) {
         // let contactList = res;
        let contactList =  res.filter(element => element.groupName != undefined);
      ...
      this._api.messagePageService('POST', url, contact).subscribe(res => {
        ...
        if (res && res['code'] == '00000') {
          ...
          if (d['exist'] || d['groupType']=="SUPPORT" && res.groupId!=undefined &&  res.groupId!=null) {
            ...
            this.sendMsgres.recipientId = d['groupId'];
            if (type === "BUSINESS" || type === "COMMUNITY" || type === "SUPPORT") {
              this.SelectedContact.newstatus = this.SelectedContact.status;
              if (this.SelectedContact.status == "ACTIVE" && res.chatStatus == "ENABLED" || this.SelectedContact.status == "ACTIVE" && res.chatStatus == "NEW") {
                this.SelectedContact.status = "ACTIVE";
              } else if (this.SelectedContact.chatStatus == "DISABLED") {
            ...
            if (type === 'USER') {
              ...
              if (groupId != undefined && groupId != null && groupId != "") {
                 this._socket.onMessage('/topic/message/' + groupId).subscribe(res => {
                  ...
                  if (type === 'BUSINESS' || type === 'COMMUNITY') {
                    console.log("Data Emitted here isss " , res , m);
                    ...
                    const messageContacts : Array<any> = res.messageContacts;

                    if(messageContacts && messageContacts.length >0){

                      messageContacts.forEach(cData =>{
                          const userId = localStorage.getItem("userId");
                          if(cData.groupId === groupId && userId === cData.userId){
                            this.SelectedContact.status = cData.chatStatus === "DISABLED" ? "INACTIVE" : cData.chatStatus;
  ...
  openNewContactModal(template?: TemplateRef<any>) {
    this.addcontact(true,template);
  }
  onscrollbusinessadminlist(){
    if (!this.stopScroll) {
      this.contact.searchAfterKey = this.previousSearchAfterKey === null ? null
      : [this.previousSearchAfterKey];
     this.businessadminlist();
  }
  }
  onsearchbusinessadminlist(){
    if (this.searchContent.trim() !== '') {
      this.contact.searchAfterKey = null
      this.previousSearchAfterKey = null
      this.businessadminlist();
  ...
  businessadminlist(){
      ...
      this._api.messagePageService('POST', url, this.contact).subscribe(res => {
        ...
        if (res) {
          ...
          const key = res.data.searchAfterKey && res.data.searchAfterKey[0];
          if (this.previousSearchAfterKey === key || (res.data.groupMembers && res.data.groupMembers.length==0)) {
           this.stopScroll = true
         }
         if (this.previousSearchAfterKey !== key) {
          if (this.searchContent != '') {
            ...
            this.groupBusinessListMembers= this.groupBusinessListMembers.concat(res.data.groupMembers) ;
            this.tempBusinessList=this.tempBusinessList.concat(res.data.groupMembers);
          }
        }
        this.previousSearchAfterKey = key;
        }
  ...
  fileUpload(event: any, template?: any, option?: any) {
    ...
    if (files[0].size > this.fileSize) {
      Swal.fire("", "Please upload file lesser than 10 MB.", "info");
      return true;
    }
  ...
  removeAttachMent(item: any, i: any) {
    ...
    this.editValueEditImage.attachments.splice(i, 1);
    if (this.uploadFiles.length === 0) {
      this.attachment = false;
    }
  }
  ...
  onScrollUpGroupmembers(){

  }

  @ViewChild('scrollCont' , {static : true}) scrolContent : any

  isBottomReached(el) {
    return (el.scrollHeight - Math.floor(el.scrollTop + 1)) === el.clientHeight;
 }
